...
func (r *RoomRepository) JoinRoom(roomID int32, userID string) error {
	return r.db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Where("id = ? AND is_private = ?", roomID, false).First(&model.RoomMatch{}).Error; err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) {
...
func (r *RoomRepository) SetReady(roomID int32, userID string, ready bool) error {
	return r.db.Transaction(func(tx *gorm.DB) error {
		...
		if count == 2 && !roomMatch.IsPrivate {
			if err := tx.Model(&model.RoomMatch{}).Where("id = ?", roomID).Update("is_private", true).Error; err != nil {
				return err
			}
			// ここにゲーム開始処理
