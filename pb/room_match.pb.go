// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.11
// 	protoc        v7.34.0
// source: proto/room_match.proto

package pb

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// Unityからは部屋名、オーナー名、プライベートかどうかが送られる
type CreateRoomMatchRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Name          string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	Owner         string                 `protobuf:"bytes,2,opt,name=owner,proto3" json:"owner,omitempty"`
	IsPrivate     bool                   `protobuf:"varint,3,opt,name=is_private,json=isPrivate,proto3" json:"is_private,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateRoomMatchRequest) Reset() {
	*x = CreateRoomMatchRequest{}
	mi := &file_proto_room_match_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateRoomMatchRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateRoomMatchRequest) ProtoMessage() {}

func (x *CreateRoomMatchRequest) ProtoReflect() protoreflect.Message {
	mi := &file_proto_room_match_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateRoomMatchRequest.ProtoReflect.Descriptor instead.
func (*CreateRoomMatchRequest) Descriptor() ([]byte, []int) {
	return file_proto_room_match_proto_rawDescGZIP(), []int{0}
}

func (x *CreateRoomMatchRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *CreateRoomMatchRequest) GetOwner() string {
	if x != nil {
		return x.Owner
	}
	return ""
}

func (x *CreateRoomMatchRequest) GetIsPrivate() bool {
	if x != nil {
		return x.IsPrivate
	}
	return false
}

// サーバーは作成された部屋のIDを返す
type CreateRoomMatchResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RoomId        int32                  `protobuf:"varint,1,opt,name=room_id,json=roomId,proto3" json:"room_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateRoomMatchResponse) Reset() {
	*x = CreateRoomMatchResponse{}
	mi := &file_proto_room_match_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateRoomMatchResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateRoomMatchResponse) ProtoMessage() {}

func (x *CreateRoomMatchResponse) ProtoReflect() protoreflect.Message {
	mi := &file_proto_room_match_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateRoomMatchResponse.ProtoReflect.Descriptor instead.
func (*CreateRoomMatchResponse) Descriptor() ([]byte, []int) {
	return file_proto_room_match_proto_rawDescGZIP(), []int{1}
}

func (x *CreateRoomMatchResponse) GetRoomId() int32 {
	if x != nil {
		return x.RoomId
	}
	return 0
}

var File_proto_room_match_proto protoreflect.FileDescriptor

const file_proto_room_match_proto_rawDesc = "" +
	"\n" +
	"\x16proto/room_match.proto\x12\troommatch\"a\n" +
	"\x16CreateRoomMatchRequest\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12\x14\n" +
	"\x05owner\x18\x02 \x01(\tR\x05owner\x12\x1d\n" +
	"\n" +
	"is_private\x18\x03 \x01(\bR\tisPrivate\"2\n" +
	"\x17CreateRoomMatchResponse\x12\x17\n" +
	"\aroom_id\x18\x01 \x01(\x05R\x06roomId2l\n" +
	"\x10RoomMatchService\x12X\n" +
	"\x0fCreateRoomMatch\x12!.roommatch.CreateRoomMatchRequest\x1a\".roommatch.CreateRoomMatchResponseB\x06Z\x04./pbb\x06proto3"

var (
	file_proto_room_match_proto_rawDescOnce sync.Once
	file_proto_room_match_proto_rawDescData []byte
)

func file_proto_room_match_proto_rawDescGZIP() []byte {
	file_proto_room_match_proto_rawDescOnce.Do(func() {
		file_proto_room_match_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_proto_room_match_proto_rawDesc), len(file_proto_room_match_proto_rawDesc)))
	})
	return file_proto_room_match_proto_rawDescData
}

var file_proto_room_match_proto_msgTypes = make([]protoimpl.MessageInfo, 2)
var file_proto_room_match_proto_goTypes = []any{
	(*CreateRoomMatchRequest)(nil),  // 0: roommatch.CreateRoomMatchRequest
	(*CreateRoomMatchResponse)(nil), // 1: roommatch.CreateRoomMatchResponse
}
var file_proto_room_match_proto_depIdxs = []int32{
	0, // 0: roommatch.RoomMatchService.CreateRoomMatch:input_type -> roommatch.CreateRoomMatchRequest
	1, // 1: roommatch.RoomMatchService.CreateRoomMatch:output_type -> roommatch.CreateRoomMatchResponse
	1, // [1:2] is the sub-list for method output_type
	0, // [0:1] is the sub-list for method input_type
	0, // [0:0] is the sub-list for extension type_name
	0, // [0:0] is the sub-list for extension extendee
	0, // [0:0] is the sub-list for field type_name
}

func init() { file_proto_room_match_proto_init() }
func file_proto_room_match_proto_init() {
	if File_proto_room_match_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_proto_room_match_proto_rawDesc), len(file_proto_room_match_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   2,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_proto_room_match_proto_goTypes,
		DependencyIndexes: file_proto_room_match_proto_depIdxs,
		MessageInfos:      file_proto_room_match_proto_msgTypes,
	}.Build()
	File_proto_room_match_proto = out.File
	file_proto_room_match_proto_goTypes = nil
	file_proto_room_match_proto_depIdxs = nil
}
